...
void MyFrame::SetupPenPanes(wxWindow *parent, wxSizer *sizer) {
  for (int i = 0; i < penCount; i++) {
    auto penPane =
        new PenSizePane(parent, wxID_ANY, i * FromDIP(4) + FromDIP(1));
...
wxPanel *MyFrame::BuildControlsPanel(wxWindow *parent) {
  ...
  controlsPanel->SetBackgroundColour(
      wxColour(isDark ? darkBackground : lightBackground));
  ...
  button->Bind(wxEVT_BUTTON,
               [this](wxCommandEvent &event) { canvas->ShowSaveDialog(); });
...
MyFrame::MyFrame(const wxString &title, const wxPoint &pos, const wxSize &size)
  ...
  canvas =
      new DrawingCanvas(splitter, wxID_ANY, wxDefaultPosition, wxDefaultSize);
